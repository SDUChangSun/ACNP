...
class TransformerModel(nn.Module):
    ...
    def forward(self, src, src_mask, dataFeat,output3):
        bpttt = src.shape[0]
        batch = src.shape[1]
        ''' if bpttt!=bptt:
        ...
        output1 = torch.cat((output1,output3),dim=2)
...
class TransformerModel2(nn.Module):
    ...
    def __init__(self, ntoken, ninp, nhead, nhid, nlayers, dropout=0.5):
        ...
        encoder_layers2 = TransformerLayer(ninp, nhead, nhid, dropout)

        self.transformer_encoder2 = TransformerModule(encoder_layers2, nlayers)

        self.encoder3 = nn.Embedding(ntoken, 128)
        self.encoder4 = nn.Embedding(MAX_OCTREE_LEVEL+1, 6)
        self.encoder5 = nn.Embedding(9, 4)


        self.ninp = ninp
        self.act = nn.ReLU()
        self.decoder2 = nn.Linear(ninp, ninp)
        self.decoder3 = nn.Linear(ninp, 1)
    ...
    def init_weights(self):
        initrange = 0.1
        self.encoder3.weight.data = nn.init.xavier_normal_(self.encoder3.weight.data )
    ...
    def forward(self, src, src_mask, dataFeat):
        ...
        aOct1 = self.encoder3(oct.long()) #a[bptt,batchsize,FeatDim(levels),EmbeddingDim]
        aLevel1 = self.encoder4(level.long())
        aOctant1 = self.encoder5(octant.long())

        a1 = torch.cat((aOct1,aLevel1,aOctant1),3)

        a1 = a1.reshape((bpttt,batch,-1))
        # src = self.ancestor_attention(a)
        src1 = a1.reshape((bpttt,a1.shape[1],self.ninp))* math.sqrt(self.ninp)
        ...
        feature = output-feature
        output = torch.cat((output,feature),dim=2)'''
        output2 = self.transformer_encoder2(src1, src_mask)

        #output = self.decoder1(self.act(self.decoder0(output)))
        output2 = 8*self.decoder3(self.act(self.decoder2(output2)))
        weights = torch.exp(-(torch.arange(8).to(device) - (output2-1 ))**2 / (2 * 1**2)).to(device)
        output3 = F.softmax(weights, dim=2)
        #output1 = torch.cat((output1,self.act(output3)),dim=2)
...
model2 = TransformerModel2(ntokens, ninp, nhead, nhid, nlayers, dropout).to(device)
if __name__=="__main__":
    ...
    if not os.path.exists(checkpointPath2):
        os.makedirs(checkpointPath2)
    ...
    # reload
    #saveDic = None
    saveDic = reload(2000200,checkpointPath2)
    if saveDic:
    ...
    def train(epoch):
        ...
        for Batch, d in enumerate(train_loader): # there are two 'BATCH', 'Batch' includes batch_size*TreePoint/batchSize/bptt 'batch'es.
            ...
            for index, i in enumerate(range(0, train_data.size(0) - 1, bptt)):
                ...
                output2,output3 = model2(data, src_mask,  dataFeat)
                o_weight = output3.detach()
                output1 = model(data, src_mask, dataFeat, o_weight)
                ...
                #loss = criterion2(output2.view(-1, 1), target1.view(-1,1).float())
                #loss = loss1+loss2
                loss.backward()
                ...
                if batch % log_interval == 0:
                    ...
                    printl('| epoch {:3d} | Batch {:3d} | {:4d}/{:4d} batches | '
                        ...
                        'loss {:5.2f} | losslist  {} | ppl {:8.2f}'.format(
                            ...
                            cur_loss,total_loss_list, math.exp(cur_loss/1000)))
                    total_loss = 0
                    start_time = time.time()
            ...
            '''if Batch%100==0:
                save(epoch*100000+Batch,saveDict={'encoder':model2.state_dict(),'idloss':idloss,'epoch':epoch,'best_val_loss':best_val_loss},modelDir=checkpointPath2)'''
