...
'''
def compress(oct_data_seq,outputfile,model,model2,actualcode = True,print=print,showRelut=False):
    model.eval()
    model2.eval()
    print('xxxx')
    ...
    oct_data_seq[:-1,0:-1,:] = oct_data_seq[1:,0:-1,:]#ignore the first node
    ...
    with torch.no_grad():
        ...
        for n,i in enumerate(trange(0, padingLength-bptt , bptt//bpttRepeatTime)):
            ...
            output2,output3 = model2(input,src_mask,[])
            output = model(input,src_mask,[],output3)
            #print(nodeID)
 ...
 # %%
def main(fileName,model,model2,actualcode = True,showRelut=True,printl = print):
    ...
    binsz,oct_len,elapsed,binszList,octNumList = compress(oct_data_seq,outputfile,model,model2,actualcode,printl,showRelut)
    if showRelut:
